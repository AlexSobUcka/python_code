...
def start_game(message):
    ...
    if secret and len(secret) == 4 and secret.isdigit() and len(set(secret)) == 4:
        bot.reply_to(message, "Game started! Please ask another user to write the number.")
    else:
...
def guess_number(message):
    ...
    if not secret:
        ...
        return
    guess = message.text
    bulls, cows, error = count_bulls_and_cows(secretbulls_cows2.py, guess)
    if error:
        bot.reply_to(message, error)
        return
    if bulls == 4:
        bot.reply_to(message, "Congratulations! You guessed the number correctly.")
        secret = None
    else:
